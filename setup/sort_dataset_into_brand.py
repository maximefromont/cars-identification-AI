...
destination_folder = "brand-dataset"

# Create the destination folder if it doesn't exist
if not os.path.exists(destination_folder):
    os.makedirs(destination_folder)

def process_file(filename,brands):
    # Split the filename into groups separated by underscores
    groups = filename.split("_")

    # Check if the first group is in the list of brands
    if groups[0] not in brands:
        return
...
def sort_dataset_into_brand(brands):    
    num_cores = cpu_count()
    
    # Create a pool of worker processes
    pool = Pool(processes=num_cores)
    process_file_with_brands = partial(process_file, brands=brands)
